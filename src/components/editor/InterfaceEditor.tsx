...
> {
        ...
        {this.state.properties ? (
          <>
            <RequestPropertyList
              ...
              mod={mod}
              interfaceId={itf.id}
              interface_type={itf.interface_type}
  ...
  handleSaveInterfaceAndProperties = (e: any) => {
    ...
    if (!itf.interface_type.trim()) {
      this.props.showMessage('接口类型不能为空', MSG_TYPE.WARNING)
      return
    }
    ...
    if (itf.interface_type === 'HTTP') {
      ...
      if (itf.url.substring(0, 4) !== 'http' && itf.url[0] !== '/') {
        this.props.showMessage(
          '合法的URL地址，需以 http 或 / 开头，例如/a/b 或者 https://www.taobao.com',
          MSG_TYPE.WARNING
        )
        return
      }
    ...
    for (const p of this.state.properties) {
      if (!p.name.trim()) {
        this.props.showMessage(
          `有参数未命名，请检查...${
            p.description ? '描述为：' + p.description : ''
          }`,
          MSG_TYPE.WARNING
        )
        return
